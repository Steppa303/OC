 ...
 * 
 * Session-Dateien haben dieses Format:
 * {"type":"message","id":"d9300351","message":{"role":"assistant","content":"..."}}
 * 
 * WICHTIG: Die ID ist im "id" Feld, NICHT "message_id"!
 */
  ...
  try {
    ...
    for (const line of lines) {
      try {
        ...
        // NUR Messages extrahieren (keine Sessions, Tools, etc.)
        if (msg.type === 'message' && msg.id) {
          messageIds.push(msg.id);
        }
        // Fallback für altes Format mit message_id
    ...
    logStatus(`📊 Extracted ${messageIds.length} message IDs from ${filepath}`);
    return messageIds;
  } catch (error) {
