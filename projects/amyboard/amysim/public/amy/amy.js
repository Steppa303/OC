...
async function setup_midi_devices() {
    if(!document.amyboard_settings || !document.amyboard_settings.midi_input) {
      return;
    }
...
async function start_midi() {
  function onEnabled() {
    // Skip MIDI setup if no form on page (e.g. our React app)
    if(!document.amyboard_settings || !document.amyboard_settings.midi_input) {
      return;
    }
    var midi_in = document.amyboard_settings.midi_input;
