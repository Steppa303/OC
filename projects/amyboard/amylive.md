...
- [x] **Cloudflare/HTTPS Setup** (proxied DNS für Secure Context)
- [x] **Caddy Config** (keine COOP/COEP → WebMIDI funktioniert auf Mobile)
- [x] **Connect-Bugfix** (`onClick={connect}` → `onClick={() => connect()}`)
...
- [ ] **Envelope 0** (ADSR mit Breakpoints, Typ-Wahl)
- [ ] **Envelope 1** (wie EG0)
- [ ] **Synth-Modul** (Voices, Patch-Wahl, MIDI CH)
- [ ] **MIDI Pads** (Keyboard zum Spielen)
- [x] Live-Parameter-Update (Slider → sofortiger Sound)
- [ ] Lokale Patch Library (Speichern/Laden/Löschen)
- [ ] State vom Board laden (zDZ)
- [ ] State auf Board schreiben (zA / zT)
- [ ] Modul-Bibliothek (Basismodule registriert)
